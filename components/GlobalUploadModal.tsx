...
const CATEGORIES = ['B&A', 'USP 강조', '리뷰', '라인업', '브랜드별', '기타']

export default function GlobalUploadModal({ onClose, onUploaded }: Props) {
  ...
  useEffect(() => {
    const fetch = async () => {
      const { data } = await supabase.from('images').select('uploader, platform, tags')
      if (data) {
        ...
        setExistingTags(Array.from(new Set(data.flatMap((d) => d.tags))))
      }
    }
    fetch()
  }, [])
  ...
  const handleSubmit = async () => {
    ...
    if (!platform.trim() && !platformInput.trim()) { setError('플랫폼을 선택하거나 입력해주세요'); return }
    if (files.length === 0) { setError('이미지를 선택해주세요'); return }
    setUploading(true)
    setError('')
    const finalPlatform = platformInput.trim() || platform

    for (let i = 0; i < files.length; i++) {
      const fd = new FormData()
      fd.append('file', files[i])
      fd.append('uploader', uploader.trim())
      fd.append('platform', finalPlatform)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      <div className="bg-white rounded-2xl border border-gray-200 w-full max-w-md mx-4 p-5 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
          ...
          {existingUploaders.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-2">
              {existingUploaders.map((u) => (
                <button key={u} onClick={() => setUploader(u)} className={`text-xs px-3 py-1 rounded-full border transition-colors ${uploader === u ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>{u}</button>
              ))}
            </div>
          )}
        ...
        <div className="mb-3">
          <label className="block text-xs text-gray-500 font-medium mb-1.5">플랫폼</label>
          {existingPlatforms.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-2">
              {existingPlatforms.map((p) => (
                <button key={p} onClick={() => { setPlatform(p); setPlatformInput('') }} className={`text-xs px-3 py-1 rounded-full border transition-colors ${platform === p && !platformInput ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>{p}</button>
              ))}
            </div>
          )}
          <input type="text" value={platformInput} onChange={(e) => { setPlatformInput(e.target.value); setPlatform('') }} placeholder="새 플랫폼 입력 (예: 세포라, 아마존)" className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:border-gray-400 outline-none transition-colors" />
        </div>
        ...
        <div className="mb-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {CATEGORIES.map((c) => (
              <button key={c} onClick={() => setCategory(c === category ? '' : c)} className={`text-xs px-3 py-1 rounded-full border transition-colors ${category === c ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>{c}</button>
            ))}
          </div>
        </div>

        <div className="mb-3">
          <label className="block text-xs text-gray-500 font-medium mb-1.5">태그 <span className="font-normal text-gray-400">(선택)</span></label>
          {existingTags.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-2">
              {existingTags.map((t) => (
                <button key={t} onClick={() => toggleTag(t)} className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${tags.includes(t) ? tagColor(t) + ' border-transparent font-medium' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}>{t}</button>
              ))}
            </div>
          )}
            ...
            <div className="flex flex-wrap gap-1.5 mt-2">
              {tags.map((t) => (
                <span key={t} className={`text-xs px-2 py-0.5 rounded-full flex items-center gap-1 ${tagColor(t)}`}>
                  {t}<button onClick={() => setTags(tags.filter((x) => x !== t))} className="opacity-60 hover:opacity-100">×</button>
                </span>
              ))}
            </div>
          )}
